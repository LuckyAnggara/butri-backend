...
class LaporanWilayahController extends BaseController
{
    public function index(Request $request)
    {
        ...
        $result = LaporanWilayah::where('group_id', Auth::user()->unit_id)
    ...
    public function debug()
    {
        ...
        foreach ($groupAll as $key => $x) {
            if ($x->id != 8) {
                $result = IndikatorKinerjaKegiatan::whereYear('created_at', $tahun)
                    ->where('group_id', $x->id)
                    ->get();
                foreach ($result as $key => $value) {
                    $value->realisasi = CapaianIndikatorKinerjaKegiatan::where('ikk_id', $value->id)
                        ->where('tahun', $tahun)
                        ->where('bulan', $bulan)
                        ->first();
                }
    ...
    public function laporanCapaianProgramUnggulan($parameter)
    {
        ...
        $breaks = ['<br />', '<br>', '<br/>'];
        $programUnggulan = ProgramUnggulan::with('list')
            ->where('tahun', $tahun)
            ->get();

        foreach ($programUnggulan as $key => $value) {
            $detail = CapaianProgramUnggulan::with('kegiatan')
                ->whereMonth('created_at', $bulan)
                ->whereYear('created_at', $tahun)
                ->where('program_unggulan_id', $value->id)
        ...
        $styleCell = ['valign' => 'center', 'size' => 11];
        $styleCellSpan = ['valign' => 'center', 'size' => 11, 'gridSpan' => 5];
        $headerTableStyle = ['bold' => true, 'align' => 'center'];
        // ADD TABLE
        ...
        foreach ($programUnggulan as $key => $value) {
            $detailPU = CapaianProgramUnggulan::with('kegiatan', 'program')
                ->where('program_unggulan_id', $value->id)
                ->where('unit_id', Auth::user()->unit_id)
                ->get();
            $number = 0;
    ...
    public function laporanPersuratan($parameter)
    {
        ...
        for ($i = 1; $i < 13; $i++) {
            ...
            $data = Persuratan::where('tahun', $tahun)
                ->where('bulan', $i)
                ->first();
            if ($data) {
                $result[] = [
    ...
    public function laporanKegiatanLainnya($parameter)
    {
        ...
        $kegiatans = Kegiatan::with('unit')
            ->where('unit_id', $unit_id)
            ->whereYear('start_at', $tahun)
            ->whereMonth('start_at', $bulan)
            ->get();
    ...
    public function laporanDataPengawasan($parameter)
    {
        ...
        foreach ($jenisPengawasan as $key => $value) {
            $detail = DataPengawasan::where('unit_id', $unit_id)
                ->where('jenis_pengawasan_id', $value->id)
                ->with('unit')
                ->where('tahun', $tahun)
    ...
    public function laporanAnggaran($parameter)
    {
        ...
        $group_id = $this->getGroup(Auth::user()->unit_id);
        // ANGGARAN
        $realisasiKegiatan = Dipa::with([
            'group',
            'realisasi' => function ($query) use ($bulan) {
    ...
    public function laporanKepegawaian($parameter)
    {
        ...
        foreach ($pangkat as $key => $value) {
            $count = Employe::where('unit_id', $unit_id)
                ->where('pangkat_id', $value->id)
                ->whereMonth('created_at', '<=', $dateQuery)
                ->get()
                ->count();
            $value->jumlah = $count;
        }
